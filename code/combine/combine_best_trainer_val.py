...
#
# Training, validation, and testing for the combined model
################################################################################
...
from combine_best_datareader import Dataset_PDB
from combine_best_fc import PotentialNetFullyConnected2
...
class Trainer:
    ...
    def init_model(self):
        in_set1_channels = self.configs["model"]["in_set1_channels"]
        in_set2_channels = self.configs["model"]["in_set2_channels"]
        self.model_final_fcnn = PotentialNetFullyConnected2(in_set1_channels, in_set2_channels, out_channels=1)

        if torch.cuda.device_count() > 0:
            self.model_final_fcnn = DataParallel(self.model_final_fcnn)
        self.model_final_fcnn.to(self.device)
    ...
    def train(self):
        ...
        train_set2_npy = self.configs["dataset"]['train_set2_npy']
        val_csvs = self.configs["dataset"]["val_csvs"]
        val_set1_npy = self.configs["dataset"]['val_set1_npy']
        val_set2_npy = self.configs["dataset"]['val_set2_npy']
        ...
        print(f"train_set1_npy: {train_set1_npy} and train_set2_npy:{train_set2_npy}")
        ...
        dataset = Dataset_PDB(train_csvs,train_set1_npy,train_set2_npy )
        val_dataset = Dataset_PDB(val_csvs,val_set1_npy,val_set2_npy )
        ...
        optimizer = optim.RMSprop(self.model_final_fcnn.parameters(), lr=self.configs["train"]["lr"])
        ...
        if isinstance(self.model_final_fcnn, (DistributedDataParallel, DataParallel)):
            model_to_save = self.model_final_fcnn.module
        else:
            model_to_save = self.model_final_fcnn
        ...
        for epoch_ind in range(epoch_start, num_epochs):
            self.model_final_fcnn.train()
            batch_losses = []
            for batch_ind, batch in enumerate(dataloader):
                set1_input = torch.stack([torch.from_numpy(x["set1_value"]).to(self.device) for x in batch])
                set2_input = torch.stack([torch.from_numpy(x["set2_value"]).to(self.device) for x in batch])
                #print(set2_input)
                ...
                pred = self.model_final_fcnn(set1_input,set2_input)
...
############### validation
            ...
            self.model_final_fcnn.eval()
            with torch.no_grad():
                for bind, batch in enumerate(val_dataloader):
                    pdbid = [x["pdbid"] for x in batch]
                    # input = [x["data"].to(self.device) for x in batch]
                    # input_efeat = [x["efeat"].to(self.device) for x in batch]
                    # input_efeat = torch.stack(input_efeat, dim=0)
                    set1_input = torch.stack([torch.from_numpy(x["set1_value"]).to(self.device) for x in batch])
                    set2_input = torch.stack([torch.from_numpy(x["set2_value"]).to(self.device) for x in batch])
                    #print(set2_input)
...
###############test
    ...
    def test(self):
        ...
        test_set2_npy = self.configs["dataset"]['test_set2_npy']
        ...
        print(f"test_f_npy: {test_set1_npy} and test_set2_npy: {test_set2_npy} ")
        # Implement the testing logic here
        test_dataset = Dataset_PDB(test_csvs,test_set1_npy,test_set2_npy)
        ...
        try:
            ...
            model_state_dict = checkpoint.pop("model_state_dict")
            self.strip_prefix_if_present(model_state_dict, "module.")
            if isinstance(self.model_final_fcnn, torch.nn.DataParallel):
                self.model_final_fcnn.module.load_state_dict(model_state_dict, strict=False)
            else:
                self.model_final_fcnn.load_state_dict(model_state_dict, strict=False)
