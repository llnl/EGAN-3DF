...
#
# Training, validation, and testing for the EGNN model
################################################################################
...
from egnn_datareader import Dataset_PDB
...
class Trainer:
    ...
    def init_model(self):
        self.model = EGNN(
                in_channels=19, out_channels=self.configs["model"]["out_dim"],
                in_edge_nf=self.configs["model"]["EGNN"]["in_edge_nf"],
                distance_cutoff=self.configs["model"]["EGNN"]["distance_cutoff"],
                n_layers=self.configs["model"]["EGNN"]["n_layers"],
                residual=self.configs["model"]["EGNN"]["residual"],
                attention=self.configs["model"]["EGNN"]["attention"],
                normalize=self.configs["model"]["EGNN"]["normalize"],
                tanh=self.configs["model"]["EGNN"]["tanh"])
    ...
    def train(self):
        ...
        optimizer = optim.RMSprop(self.model.parameters(), lr=self.configs["train"]["lr"])
